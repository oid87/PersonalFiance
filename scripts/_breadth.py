"""Shared breadth computation for the fetch_breadth*.py scripts (S&P 500 / Nasdaq-100 /
TW50 / XLG). Constituent-fetching differs per index and stays in each fetch_breadth_*.py;
this module holds the common price-download + rolling-MA-breadth + merge + run flow,
migrated verbatim from fetch_breadth.py and parameterized via run().
"""
...
def compute_breadth(price_df: pd.DataFrame, *, min_coverage: int) -> list[dict]:
...
def run(*, get_tickers: Callable[[], list[str]], out_path: Path, min_coverage: int, label: str) -> None:
    ...
    tickers = get_tickers()
    print(f"{label} constituent count: {len(tickers)}")
